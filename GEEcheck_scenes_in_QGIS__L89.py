...
# T1
subCol1 = L89.filterBounds(ROI).filterDate(timeStart.advance(-1,'years'), timeStop.advance(-1,'years')).sort('system:time_start')
print(subCol1.size().subtract(1).getInfo())
IMG1 = ee.Image(subCol1.map(scaleL89).toList(100).get(9))
Map.addLayer(IMG1, L89Viz, "T1", False)

# T2
subCol2 = L89.filterBounds(ROI).filterDate(timeStart, timeStop).sort('system:time_start')
print(subCol2.size().subtract(1).getInfo())
IMG2 = ee.Image(subCol2.map(scaleL89).toList(100).get(9))
Map.addLayer(IMG2, L89Viz, "T2")

# T3
subCol3 = L89.filterBounds(ROI).filterDate(timeStart.advance(1,'years'), timeStop.advance(1,'years')).sort('system:time_start')
print(subCol3.size().subtract(1).getInfo())
IMG3 = ee.Image(subCol3.map(scaleL89).toList(100).get(9))
